...
from models import User, Dnslog
...
async def read_items(page: int = 1, limit: int = 10, sort: str = '-id', token: str = Depends(access_token)):
    ...
    result = {
        ...
        'items': user_obj.dnslog_item[::sort_id][(page-1)*limit:limit*page]
    }
...
async def delete_items(size: int = 10, token: str = Depends(access_token)):
    try:
        user_obj = Session.query(User).filter_by(token=token).first()
        sort_id = -1
        for i in user_obj.dnslog_item[::sort_id][:size]:
            Session.delete(i)
        Session.commit()
        return {'code': 20000}
...
async def delete_all_items(token: str = Depends(access_token)):
    try:
        user_obj = Session.query(User).filter_by(token=token).first()
        Session.query(Dnslog).filter_by(owner_id=user_obj.id).delete()
        Session.commit()
        return {'code': 20000}
    except Exception as e:
        print(e)
